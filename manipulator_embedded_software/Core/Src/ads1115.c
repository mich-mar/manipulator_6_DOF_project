...
int16_t ADS1115_ReadADC_SingleEnded(uint8_t *device_address, uint8_t channel, ADS1115_Status *status)
{
    ...
    ADS1115_Debug.last_voltage = ADS1115_ConvertToVoltage(result, ADS1115_PGA_6_144V);
...
void ADS1115_ReadAllValues(ADS1115_Readings *ADCdata)
{
    ...
    voltage = ADS1115_ConvertToVoltage(adc_value, ADS1115_PGA_6_144V);
    ADCdata->A1_adc_raw = adc_value;
    ADCdata->A1_adc = voltage; // done
    ADCdata->joint_angle_1 = calculateJointAngle(adc_value, 1);

    // kanał A1
    adc_value = ADS1115_ReadADC_SingleEnded(&address_0_3, 1, NULL);
    voltage = ADS1115_ConvertToVoltage(adc_value, ADS1115_PGA_6_144V);
    ADCdata->A2_adc_raw = adc_value;
    ADCdata->A2_adc = voltage; // done
    ADCdata->joint_angle_2 = calculateJointAngle(adc_value, 2);
    ...
    ADCdata->joint_angle_4 = calculateJointAngle(adc_value, 4);
    ...
    ADCdata->joint_angle_3 = calculateJointAngle(adc_value, 3);
    ...
    ADCdata->A5_adc = voltage; // done
    ADCdata->joint_angle_5 = calculateJointAngle(adc_value, 5);

    // kanał A5 (A1 na drugim ADS1115)
    adc_value = ADS1115_ReadADC_SingleEnded(&address_4_5, 1, NULL);
    voltage = ADS1115_ConvertToVoltage(adc_value, ADS1115_PGA_6_144V);
    ADCdata->A0_adc_raw = adc_value;
    ADCdata->A0_adc = voltage; // done
    ADCdata->joint_angle_0 = calculateJointAngle(adc_value, 0);
}
...
/**
 * @brief Przelicza wartość surową ADC na kąt dla danego przegubu
 * @param raw_value Wartość surowa z ADC (0-RAW_MAX_VALUE_5V)
 * @param joint_number Numer przegubu (0-5)
 * @return Kąt w stopniach (-270 do +270), gdzie 0 to pozycja środkowa
 */
float calculateJointAngle(uint16_t raw_value, uint8_t joint_number)
{
    // Wartość środkowa dla danego przegubu
    uint16_t middle_value;

    // Wybierz wartość środkową na podstawie numeru przegubu
    switch (joint_number)
    {
    case 0:
        middle_value = JOIN_MIDDLE_0;
        break;
    case 1:
        middle_value = JOIN_MIDDLE_1;
        break;
    case 2:
        middle_value = JOIN_MIDDLE_2;
        break;
    case 3:
        middle_value = JOIN_MIDDLE_3;
        break;
    case 4:
        middle_value = JOIN_MIDDLE_4;
        break;
    case 5:
        middle_value = JOIN_MIDDLE_5;
        break;
    default:
        return 0.0f; // Zwróć 0 dla nieprawidłowego numeru przegubu
    }

    // Oblicz różnicę między wartością aktualną a środkową
    int32_t difference = (int32_t)raw_value - (int32_t)middle_value;

    // Oblicz kąt
    // RAW_MAX_VALUE_5V odpowiada 270 stopniom
    float angle = (float)difference * MAX_ANGLE / RAW_MAX_VALUE_5V;
